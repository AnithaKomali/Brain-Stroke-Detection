...
app=Flask(__name__)
app.config['UPLOAD_FOLDER']=r"E:\pycharm YMTS\heart stroke\upload"
...
def XG_Boost():
        ...
        d1['XGBClassifier']=[n11,n12,n13,n14]
        print(d1)
        ...
        global d
        d['XGBClassifier']=[m11, m12, m13, m14]
        model.fit(x_train_sm, y_train_sm)
        y_pred = model.predict(X_test)
        acc_km = accuracy_score(y_test, y_pred)
        flash("XGBClassifier performed Successfully", 'secondary')
        return acc_km
...
def LGBM_Classifier():
    ...
    kfold = KFold(n_splits=10, random_state=99)
    loo = LeaveOneOut()
    classifier = LGBMClassifier(n_neighbors=8)
    smt = SMOTE()
    x_train_sm, y_train_sm = smt.fit_resample(X_train, y_train)
    loo.get_n_splits(x_train_sm)
    ...
    d1['LGBMClassifier'] = [n31, n32, n33, n34]
    ...
    d["LGBMClassifier"] = [m31, m32, m33, m34]
    classifier.fit(x_train_sm, y_train_sm)
    y_pred = classifier.predict(X_test)
    acc_gnb = accuracy_score(y_test, y_pred)
    flash("LGBMClassifier model created Successfully", 'secondary')
    return acc_gnb
